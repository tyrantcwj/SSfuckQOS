...
- 容器内 `SERVER_PORT=80`、`ADMIN_PORT=8080` **不要改**
- 客户端 `REMOTE_PORT` 填 **本地端口 5280**（不是 80）
- 光猫/上级路由把公网 `5280` 转到爱快 `5280`
...
`REMOTE_PORT` 填服务端的 **本地端口 5280**；密码 / path 与服务端一致。
...
| 现象 | 处理 |
|------|------|
| 客户端连不上服务端 | 看客户端日志有无 `TCP FAIL`；`REMOTE_PORT` 填**本地端口**（如 5280），不是容器内 80；密码/path 一致；光猫转发公网 5280→爱快 5280 |
