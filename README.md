...
- **禁止** 命名卷 → 只用宿主机路径挂载（如 `/etc/ikuai/docker/...`）

> 镜像若 pull 失败：先把 GHCR 包设为 Public，或在爱快里对 `ghcr.io` 登录 GitHub Token。  
> 部署前可在爱快「文件」里先建目录：`/etc/ikuai/docker/ssfuckqos/server`、`/etc/ikuai/docker/ssfuckqos/client`。
...
services:
  ssfuckqos-server:
    ...
    ports:
      ...
      - "8080:8080"
    volumes:
      - /etc/ikuai/docker/ssfuckqos/server:/data
```

说明：

1. 爱快若已占用 `80`，把两边都改成例如 `8088:80`，并保证 `SERVER_PORT` 仍是容器内 `80`；或把 `SERVER_PORT` 和映射一并改成 `8088`
...
services:
  ssfuckqos-client:
    ...
    ports:
      ...
      - "8118:8118"
    volumes:
      - /etc/ikuai/docker/ssfuckqos/client:/data
```
...
| 爱快提示命名卷/host 禁止 | 用 README 爱快编排：路径挂载 + `ports`，不要用 named volume / host |
