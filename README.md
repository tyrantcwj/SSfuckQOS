...
- **禁止** 命名卷 → 必须用文件路径挂载
- **禁止** 绝对路径（如 `/etc/...`）→ 挂载必须在编排目录内，用相对路径 `./xxx`
...
1. `./data` 会落在爱快该编排项目目录下，不要改成 `/etc/...` 绝对路径
2. 爱快若已占用 `80`，用下面 `8088` 版本
3. 公网 / 上级路由需把对应端口转到这台爱快
4. 桥接下访问家里 `10.0.x.x` 一般仍可通过爱快转发；若管理页「局域网探测」失败，检查爱快 Docker 网络是否允许访问内网
